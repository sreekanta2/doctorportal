...
function LockLink({
  ...
  if (subItem.badge) {
    return (
      <span className="text-base flex space-x-3 items-center transition-all duration-150 opacity-50  cursor-not-allowed">
        <span
...
const SubMenuItem = ({ subItem }: { subItem: any }) => {
  ...
  return (
    <LockLink href={subItem.href} subItem={subItem}>
      <div
        className={cn(
          "text-base capitalize  font-normal flex gap-3 items-center transition-all duration-150 rounded dark:hover:text-primary  ",
          {
